...
public class PreluareTemperatura extends AppCompatActivity {
    private static final String TAG = "BlueTest5-Controlling";
    private int mMaxChars = 50000;
    private UUID mDeviceUUID;
    private BluetoothSocket mBTSocket;
    private PreluareTemperatura.ReadInput mReadThread = null;
    private boolean mIsUserInitiatedDisconnect = false;
    private boolean mIsBluetoothConnected = false;
    private BluetoothDevice mDevice;
    private ProgressDialog progressDialog;
    private Button buttonbdTemp;
    GraphView graph;
    public int coordonataX = 0;
    LineGraphSeries<DataPoint> series;
    DatabaseReference reff = FirebaseDatabase.getInstance().getReference();
    BazaDateTemperatura bdTemp;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonbdTemp = (Button) findViewById(R.id.addTempPacientBtn);
        textViewTemp = findViewById(R.id.textviewTemp);
        bdTemp = new BazaDateTemperatura();
        graph = (GraphView) findViewById(R.id.grafTemp);
        ...
        buttonbdTemp.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                bdTemp.setValue(String.valueOf(temp_final));
                bdTemp.setData(date);
                bdTemp.setId(id);
                Log.d("userID", userId);
                reff.child("Conturi").child("Pacienti").child(userId).child("ValoriTemperatura").child(String.valueOf(id)).setValue(bdTemp);

                Toast.makeText(PreluareTemperatura.this, "Data inserted successfully", Toast.LENGTH_LONG).show();
            }
        });
                            ...
                            public void run() {
                                ...
                                if (despartit.length > 1) {
                                    if (despartit[2].length() > 0 && despartit[2].contains("T=")) {
                                        temp = Integer.parseInt(despartit[1].split("=")[2]);
                                        Log.d("string_temp", String.valueOf(despartit[1]));
                                        textViewTemp.setText("Temperatura dumneavoastra este: " + temp);
                                        temp_final = temp;
                                        series.appendData(new DataPoint(coordonataX++, temp), true, 10);

//                                        if (puls > 50 && puls < 60) {
//                                            contor5060++;
//                                            if (contor5060 == 10) {
//                                                series.appendData(new DataPoint(coordonataX++, puls), true, 10);
//                                                textViewTemp.setText("Pulsul final este: " + puls);
//                                                puls_final = puls;
//                                                t.interrupt();
...
//                                        }
    ...
    protected void onPause() {
        if (mBTSocket != null && mIsBluetoothConnected) {
            new PreluareTemperatura.DisConnectBT().execute();
        }
        Log.d(TAG, "Paused");
        super.onPause();
    }
    ...
    protected void onResume() {
        if (mBTSocket == null || !mIsBluetoothConnected) {
            new PreluareTemperatura.ConnectBT().execute();
        }
        Log.d(TAG, "Resumed");
        super.onResume();
    ...
    private class ConnectBT extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute() {
            ...
            progressDialog = ProgressDialog.show(PreluareTemperatura.this, "Hold on", "Connecting");
        ...
        protected void onPostExecute(Void result) {
            ...
            if (!mConnectSuccessful) {
                ...
                mReadThread = new PreluareTemperatura.ReadInput();
            }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event)
    {
        if ((keyCode == KeyEvent.KEYCODE_BACK))
        {
            ...
            startActivity(new Intent(getApplicationContext(), PreluareTemperatura.class));
